...
const Nav = () => {
    ...
    return (
        ...
        <div className='w-full py-3 md:py-0 dark:bg-brand-dark dark:text-brand font-mono text-sm'>
            ...
            {/* Desktop Menu */}
            <nav>
                <ul className="hidden md:fixed md:flex md:border-r md:dark:border-gray-600 flex-col justify-between items-center h-full w-max">

                    {links.map(l => (
                        <li key={l.name} className='w-full h-full flex items-center justify-center dark:bg-brand-dark hover:bg-gray-100 dark:hover:bg-brand-dark/80'>
                            <Link href={l.url}>
                                <a
                                    href={l.url}
